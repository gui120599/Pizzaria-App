  ...
  <div x-data="{ showCliente: false, showPagamento: false }"
    ...
    <div class="py-2 px-2 sm:px-4 pb-28">
        ...
        <div class="flex items-center gap-3 mb-4">
            ...
            <h1 class="text-lg font-bold text-gray-800">
                Nova Venda <span id="venda_id_titulo" class="text-teal-600"></span>
            </h1>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-5 gap-4">
            ...
            <div class="xl:col-span-2 flex flex-col gap-4">
                ...
                <form id="formVenda" action="{{ route('venda.store') }}" method="POST">
                    @csrf
                    <input type="hidden" name="venda_id" id="venda_id" value="">
                    <input type="hidden" name="venda_sessao_caixa_id" id="venda_sessao_caixa_id" value="{{ $sessaoCaixa->id }}">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="bg-white shadow-sm rounded-xl p-4">
                            ...
                            <div class="grid grid-cols-3 gap-2 text-xs">
                                <div>
                                    <span class="block font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Sessão</span>
                                    <span class="text-gray-800 font-medium">#{{ $sessaoCaixa->id }}</span>
                                </div>
                                <div>
    ...
    <script type="module">
        $(document).ready(function () {
            ...
            // Auto-cancela venda iniciada e zerada ao sair/recarregar
            window.addEventListener('beforeunload', function () {
                const id    = vendaId();
                const total = parseFloat(($("#venda_valor_total").val() || '0').replace(/\./g, '').replace(',', '.')) || 0;
                if (id && total === 0) {
                    navigator.sendBeacon(
                        "{{ route('venda.cancelar') }}",
                        new Blob([new URLSearchParams({ '_token': '{{ csrf_token() }}', venda_id: id }).toString()],
