<section>
            ...
            <div class="col-span-full md:col-span-3">
                <div class="h-96 overflow-auto">
                    <table class="w-full text-[7px] md:text-base">
                        <thead class="sticky top-0 text-start bg-white p-4 border-b m-1">
                        ...
                        <tbody>
                            @foreach ($adicionais as $adicional)
                                ...
                                <tr>
                                    <td>
                                        <i @if ($isActive) class='bx bxs-toggle-right text-3xl cursor-pointer toggle-adicional' style='color:#00fd83'
                                        @else
                                        class='bx bx-toggle-left text-3xl cursor-pointer toggle-adicional' @endif
                                            title="Ativar" data-adicional_id="{{ $adicional->id }}"
                                            data-produto_id="{{ $produto->id }}"></i>
                                    </td>
                                    ...
                                    <td><span>{{ $adicional->adicional_nome }}</span></td>
    ...
    <script type="module">
        $(document).ready(function() {
            //Função atualiza os adicionais do produto
            ...
            // Função que ativa o adicional no produto
            $('.toggle-adicional').click(function(e) {
                e.preventDefault();
                const isActive = $(this).hasClass('bxs-toggle-right');
                const adicionalId = $(this).data('adicional_id');
                const produtoId = $(this).data('produto_id');
                if (isActive) {
                    $(this).removeClass('bxs-toggle-right');
                    $(this).addClass('bx-toggle-left');
                    $(this).css('color', '#000000');
                } else {
                    $(this).removeClass('bx-toggle-left');
                    $(this).addClass('bxs-toggle-right');
                    $(this).css('color', '#00fd83');
                }
                $.ajax({
                    type: "POST",
                    url: "{{ route('adicional_produto.ativar') }}",
                    data: {
                        isActive,
                        adicionalId,
                        produtoId,
                    ...
                    success: function(response) {
                        console.log(response.message); // Mensagem no console
                    },
                    error: function(e) {
