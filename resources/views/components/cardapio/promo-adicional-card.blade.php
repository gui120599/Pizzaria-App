...
<div class="relative">
    <a href="{{ route('produto.show', ['produto' => $produto]) }}">
        <div class="w-full p-2 rounded-lg flex items-start justify-between gap-1 border border-pink-500/40 bg-gradient-to-br from-pink-500/10 to-fuchsia-500/5">
            <div class="w-2/5 relative">
                <img src="{{ $produto->getImagemUrl() }}" alt="{{ $produto->produto_descricao }}"
                     class="w-32 h-28 object-cover rounded-lg bg-white">
                <span class="absolute top-1 left-1 bg-pink-600 text-white text-[10px] font-bold px-1 py-0.5 rounded flex items-center gap-0.5">
                    <i class='bx bxs-gift text-xs'></i> OFERTA
                </span>
            </div>
            <div class="w-full h-28 flex flex-col justify-center space-y-1">
                ...
                <span class="text-pink-300 text-[11px] leading-tight">
                    🎁 Leve também:
                    @foreach ($ofertas as $oferta)
                        {{ $oferta['nomeExibicao'] }} (+R${{ number_format($oferta['valorAdicional'], 2, ',', '.') }}){{ ! $loop->last ? ' ou ' : '' }}
                    @endforeach
                </span>
            </div>
        </div>
