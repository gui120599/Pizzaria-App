...
<head>
    ...
    <title>Relatório de Pedidos Mensais</title>
...
<body class="flex justify-center m-0 text-xs">
    <div id="conteudo" class="p-1 mt-1 w-full max-w-md">
        ...
        <p class="text-center text-sm font-bold mb-2 uppercase">Relatório de Pedidos Mensais</p>
        ...
        <table class="w-full border border-gray-300">
            <thead>
                <tr class="bg-gray-200">
                    ...
                    <th class="border p-1 text-right">Total de Pedidos (R$)</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($pedidosMensais as $pedido)
                    <tr>
                        <td class="border p-1 uppercase">
                            {{ \Carbon\Carbon::createFromFormat('Y-m-d', $pedido->mes . '-01')->translatedFormat('F/Y') }}
                        </td>
                        <td class="border p-1 text-right font-bold">
                            R$ {{ number_format($pedido->total_pedidos, 2, ',', '.') }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="2" class="border p-2 text-center italic">Nenhuma pedido encontrada.</td>
                    </tr>
                @endforelse
                <tr class="bg-gray-100">
                    <td class="border p-1 font-bold">Total Geral</td>
                    <td class="border p-1 font-bold text-right">
                        R$ {{ number_format($pedidosMensais->sum('total_pedidos'), 2, ',', '.') }}
                    </td>
                </tr>
            </tbody>
        </table>
